...
#  results is the summary table from WinBugs
   ...
   points(time.H, log(Uguess.H.1[time>hatch.after]), pch="h", cex=.5)
   ...
   points(time.H+.2, etaU.H.1  [,"mean"], type="p", pch=22)  # fitted values for hatchery fish
   ...
   lines(time.H+.2, etaU.H.1  [,"mean"], lty=3, lwd=2)
   ...
   segments(time.H+.2, etaU.H.1  [,"2.5%"], time.H+.2, etaU.H.1  [,"97.5%"], lty=3, lwd=2)
   ...
   lines(time.H, logUne.H.1,   lty=3, lwd=2)  # plot the curve # too busy to plot the spline points as well

}

plot_logitP <- function(title, time, n1, m2, u2.W.YoY, u2.W.1, u2.H.1, logitP.cov, results){
...
plot_logitP(title=title, time=new.time, n1=new.n1, m2=new.m2, u2.W.YoY=new.u2.W.YoY, u2.W.1=new.u2.W.1, u2.H.1=new.u2.H.1,
...
q <- RunTime(time=time[time>hatch.after], U=results$sims.list$U.H.1, prob=run.prob)
temp <- rbind(apply(q,2,mean), apply(q,2,sd))
rownames(temp) <- c("Mean", "Sd")
print(round(temp,2))
