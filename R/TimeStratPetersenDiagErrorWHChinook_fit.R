...
#  points(time.H, logUne.H, type="p", pch=21)
   lines(time.H, logUne.H, lty=2)  # plot the curve # too busy to plot the spline points as well

}

plot_logitP <- function(title, time, n1, m2, u2.A, u2.N, logitP.cov, results){
...
plot_logitP(title=title, time=new.time, n1=new.n1, m2=new.m2, u2.A=new.u2.A, u2.N=new.u2.N,logitP.cov=new.logitP.cov, results=results) 
dev.off()
...
cat("Wild\n")
print(round(results$summary[ grep("Utot.W", rownames(results$summary)),]))
cat("\n\nHatchery\n")
print(round(results$summary[ grep("Utot.H", rownames(results$summary)),]))
cat("\n\nTotal\n")
...
q <- RunTime(time=time[time>hatch.after], U=results$sims.list$U.H, prob=run.prob)
temp <- rbind(apply(q,2,mean), apply(q,2,sd))
rownames(temp) <- c("Mean", "Sd")
print(round(temp,2))
